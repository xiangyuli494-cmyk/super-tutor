...
from super_tutor.core.orchestrator_utils import _hydrate_models, _safe_parse_json_list
...
class _PhaseHandlers:
    ...
    async def _quiz_gen_phase(self: "Orchestrator") -> None:
        ...
        输入：_artifacts["chunks"] + mastery_records（自适应出题）
        产出：Question 列表 + QuizSession，写入 _artifacts["questions"]
        下一状态：EVALUATING

        自适应出题逻辑：
        1. 查询学生历史掌握度数据（mastery_records）
        2. 将掌握度摘要注入出题 prompt
        3. LLM 将题目集中在薄弱知识点上
        """
        ...
        try:
            ...
            if student_id:
                try:
                    ...
                    if mastery_records:
                        # 收集 chunks 中的 knowledge_node_ids
                        chunk_node_ids: set[str] = set()
                        for c in chunks:
                            node_ids = c.get("knowledge_node_ids", [])
                            if isinstance(node_ids, list):
                                chunk_node_ids.update(node_ids)

                        # 构建节点→掌握度的映射
                        node_mastery: dict[str, dict[str, Any]] = {}
                        for rec in mastery_records:
                            nid = rec.get("knowledge_node_id", "")
                            if nid:
                                node_mastery[nid] = rec

                        # 按掌握度排序：弱 → 强
                        sorted_nodes = sorted(
                            node_mastery.items(),
                            key=lambda kv: kv[1].get("mastery_level", 0.0),
                        )

                        # 生成掌握度摘要（供 LLM 理解）
                        lines: list[str] = []
                        for nid, rec in sorted_nodes:
                            ml = rec.get("mastery_level", 0.0)
                            pct = round(ml * 100)
                            state = rec.get("state", "new")
                            attempts = rec.get("total_attempts", 0)
                            correct = rec.get("correct_attempts", 0)
                            mis_ids = rec.get("misconception_ids", "[]")

                            # 解析迷思概念 ID 列表
                            try:
                                mis_list = (
                                    _json.loads(mis_ids)
                                    if isinstance(mis_ids, str)
                                    else mis_ids
                                )
                            except Exception:
                                mis_list = []

                            # 生成节点描述（优先从 chunks 中找 topic）
                            node_label = nid
                            for c in chunks:
                                if nid in (c.get("knowledge_node_ids") or []):
                                    topic = c.get("topic", "")
                                    if topic:
                                        node_label = f"{nid}（{topic}）"
                                    break
                            ...
                            line = (
                                f"- {node_label}: 掌握度 {pct}% "
                                f"（{state_label}，{correct}/{attempts} 次正确）"
                            )
                            if mis_list:
                                line += f"，存在 {len(mis_list)} 个迷思概念需纠正"
                            lines.append(line)

                        # 新增节点（chunks 中有但 mastery 中没有的）
                        new_nodes = chunk_node_ids - set(node_mastery.keys())
                        for nid in sorted(new_nodes):
                            ...
                            for c in chunks:
                                if nid in (c.get("knowledge_node_ids") or []):
                                    ...
                                    break
                            lines.append(
                                f"- {node_label}: 无历史数据（视同弱知识点，重点出题）"
                            )

                        mastery_summary = "\n".join(lines)
                        logger.info(
                            "QUIZ_GEN: 已加载 %d 条掌握度记录用于自适应出题 "
                            "（%d 个已知节点, %d 个新节点, student_id=%r）。",
                            len(mastery_records),
                            len(node_mastery),
                            len(new_nodes),
                            student_id,
                    ...
                    logger.warning(
                        "QUIZ_GEN: 无法查询 mastery_records (%s)，"
                        "回退到均匀出题模式。",
                        exc,
                    )
                    mastery_summary = ""

            prompt = _build_quiz_gen_prompt(chunks, mastery_summary=mastery_summary)
